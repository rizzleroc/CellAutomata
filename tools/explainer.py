...
SCENES = [
    dict(kind="rd", relit=True, kw=dict(preset="mitosis"), seed="central", grid=150, pal="copper",
         cap="It's only math",
         vo="Life doesn't need D N A to begin."),
    dict(kind="rd", relit=False, kw=dict(F=0.030, k=0.057), seed="scatter", grid=220, pal="cyto",
         cap="Simple rules, wild results",
         vo="Give a grid a dead-simple rule, and watch what crawls out."),
    dict(kind="rd", relit=False, kw=dict(preset="mitosis"), seed="central", grid=150, pal="membrane",
         cap="Self-replicating cells",
         vo="Some patterns copy themselves, dividing like real cells."),
    dict(kind="grid", cap="16-way parallel search",
         vo="Sixteen searches run in parallel, hunting for the most alive-looking patterns."),
    dict(kind="rd", relit=True, kw=dict(F=0.018, k=0.050), seed="scatter", grid=220, pal="amethyst",
         cap="One engine, endless forms",
         vo="From the same rules come snowflakes, mandalas, and growing colonies."),
    dict(kind="mandala", kw=dict(F=0.026, k=0.055), n1=12, n2=6, pal="nebula",
         cap="Reproducible from a seed",
         vo="Every shape is born from a fixed, deterministic seed."),
    dict(kind="rd", relit=True, kw=dict(F=0.026, k=0.055), seed="scatter", grid=220, pal="gold",
         cap="No artist · 4K art",
         vo="No animator, no randomness — just math, rendered in four K."),
    dict(kind="rd", relit=True, kw=dict(preset="mitosis"), seed="central", grid=150, pal="copper",
         cap="Open source · watch & subscribe",
         vo="A sandbox for the origin of life — open source, and waiting for you."),
]
