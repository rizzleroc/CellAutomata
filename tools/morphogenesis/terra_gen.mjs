// TERRA — the field read as living terrain. Coastlines, ranges, archipelagos:
// labyrinth -> coral -> spots -> stripes -> labyrinth -> coral. Many small seeds so the
// "continents" sprawl and reorganise. NF overridable for smoke tests:  MORPH_NF=48 node ...
import fs from 'fs';
...
// Terrain morphologies — all in the alive band. labyrinth=ranges, coral=archipelago, spots=islands, stripes=ridges.
const WP=[
...
// Many small scattered seeds — 120 nuclei, radius 5 — so continents sprawl across the whole plate.
const rng=mul(51790088);
for(let i=0;i<120;i++) g.paint((rng()*W)|0,(rng()*H)|0,5,'paint');
const fd=fs.openSync('/tmp/terra_field.bin','w'); const fk=[],names=[],pop=[]; const t0=Date.now();
for(let f=0;f<NF;f++){
...
fs.closeSync(fd);
fs.writeFileSync('/tmp/terra_meta.json',JSON.stringify({W,H,NF,SEG,WP,fk,names,pop}));
