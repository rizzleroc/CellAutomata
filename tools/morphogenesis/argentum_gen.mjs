// ARGENTUM — a scanning-electron-micrograph plate. A specimen that divides then
// differentiates into tissue: spots -> mitosis -> coral -> labyrinth -> coral -> spots.
// Moderate scatter so the plate fills with a believable specimen texture.
// NF overridable for smoke tests:  MORPH_NF=48 node ...
import fs from 'fs';
...
// Cell division -> tissue differentiation. All waypoints in the alive band.
const WP=[
 ['spots',0.030,0.062],
...
// Moderate scatter — 110 nuclei, radius 6. A specimen that fills the field.
const rng=mul(22446688);
for(let i=0;i<110;i++) g.paint((rng()*W)|0,(rng()*H)|0,6,'paint');
const fd=fs.openSync('/tmp/argentum_field.bin','w'); const fk=[],names=[],pop=[]; const t0=Date.now();
for(let f=0;f<NF;f++){
...
fs.closeSync(fd);
fs.writeFileSync('/tmp/argentum_meta.json',JSON.stringify({W,H,NF,SEG,WP,fk,names,pop}));
