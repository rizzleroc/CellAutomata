// RADIATING — a chromatic mandala. The morphogenetic field is bathed in spectral rings that
// emanate outward from the core; growth via spots/mitosis/coral so the structure radiates too.
import fs from 'fs';
...
const WP=[
 ...
 ['coral',0.0545,0.062],
 ['mitosis',0.0366,0.0648],
 ['spots',0.031,0.062]
];
const NF=+(process.env.MORPH_NF||2880), SEG=WP.length-1;
setF(WP[0][1],WP[0][2]); g.reset();
const rng=mul(80706050);
for(let i=0;i<100;i++) g.paint((rng()*W)|0,(rng()*H)|0,5,'paint');
const fd=fs.openSync('/tmp/radiating_field.bin','w'); const fk=[],names=[],pop=[]; const t0=Date.now();
for(let f=0;f<NF;f++){
...
fs.closeSync(fd);
fs.writeFileSync('/tmp/radiating_meta.json',JSON.stringify({W,H,NF,SEG,WP,fk,names,pop}));
