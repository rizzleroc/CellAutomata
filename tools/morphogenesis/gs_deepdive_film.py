"""DEEP DIVE — the most incredible Gray-Scott patterns a swarm of 16 agents found across ~1,000
points of F/k space. Twelve champions, each matured then shown LIVING (relief-shaded, aurora
palette, slow push-in), labelled with its discovered F/k. Reads /tmp/dd_field.bin (gs_deepdive_gen).
Preview: python3 gs_deepdive_film.py test <globalframe>   Full: python3 gs_deepdive_film.py"""
...
def roman(n):
    ...
    for v,sym in ROMAN:
        while n>=v: s+=sym; n-=v
    return s
M=json.load(open('/tmp/dd_meta.json')); R=M['R']; MF=M['M']; PAT=M['patterns']; NP=len(PAT)
SZ=R*R*2
def readfield(idx,lf):
    g=idx*MF+min(MF-1,max(0,lf)); f=open('/tmp/dd_field.bin','rb'); f.seek(g*SZ)
...
def relief(h01):
    ...
    # stretch the GS band (v ~0..0.4) across the LUT for vivid colour
...
def window(idx,lf,t):
    a=readfield(idx,lf)
    zoom=1.04+0.26*t; cs=int(round(R/zoom)); cs=max(40,min(R,cs))
    off=int((R-cs)*0.5+(R-cs)*0.16*np.sin(t*np.pi)); x=max(0,min(R-cs,off)); y=max(0,min(R-cs,int((R-cs)*0.5)))
    sub=a[y:y+cs,x:x+cs]
    h01=np.asarray(Image.fromarray(sub,mode='F').resize((WIN,WIN),Image.LANCZOS),np.float32)
    return np.clip(relief(h01).astype(np.float32)*VIG,0,255).astype(np.uint8)
...
TITLE=96; SEG=MF; CODA=120; FADE=12
...
def compose(f):
    ...
    if kind=='title':
        ...
        label(cv,(W//2,560),"gray–scott · F/k parameter space",22,AC,0.7*g)
        text(cv,(W//2,840),"DEEP DIVE",F_disp(132),BONE,g)
        text(cv,(W//2,990),"the most incredible patterns one equation can make",F_ital(42),AC,g)
        label(cv,(W//2,1330),"found by a swarm of 16 agents · ~1,000 points searched",19,DIM,0.7*g)
        return cv
    if kind=='coda':
        g=min(1,lf/18)*min(1,(CODA-1-lf)/16)
        label(cv,(W//2,560),"two numbers · twelve worlds",20,AC,0.65*g)
        text(cv,(W//2,820),"one rule,",F_disp(60),BONE,g)
        text(cv,(W//2,910),"endless form",F_disp(60),AC,g)
        label(cv,(W//2,1300),"du = Du lap(u) - uv^2 + F(1-u)",18,DIM,0.6*g)
        return cv
    ...
    text(cv,(W//2,250),p['name'].upper(),F_disp(60),BONE,a)
    label(cv,(W//2,322),f"F {p['F']:.4f}   ·   k {p['k']:.4f}",18,AC,0.7*a)
    wrapped(cv,(W//2,WY+WIN+64),p['cap'],F_ital(38),BONE,a,W-150,50)
    for j in range(NP):
        x=W//2-(NP-1)*16+j*32; r=5 if j==k else 3; fill=AC if j==k else DIM
        d.ellipse([x-r,1858-r,x+r,1858+r],fill=(*fill,255 if j==k else 110))
    label(cv,(W//2,1900),"cellautomata · deep dive",14,DIM,0.4)
    return cv
if __name__=='__main__' and len(sys.argv)>2 and sys.argv[1]=='test':
    compose(int(sys.argv[2])).convert("RGB").save('/tmp/dd_test.png'); print("NF",NF,"patterns",NP); sys.exit()
# ===== FULL RENDER =====
silent="/tmp/dd_silent.mp4"
...
total=NF/FPS; out="/tmp/web8_deepdive.mp4"
fade=min(3.5,total/3); fin=min(3.0,total/3); fo=max(0.0,total-fade)
af=f"[1:a][2:a][3:a]amix=inputs=3,volume=0.11,lowpass=f=540,afade=t=in:st=0:d={fin:.2f},afade=t=out:st={fo:.2f}:d={fade:.2f}[a]"
