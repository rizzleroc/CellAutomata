"""DEEP DIVE II — deeper, smoother, more incredible. The 8 best Gray-Scott champions at 512²,
each a slow eased descent INTO the texture: relief-shaded aurora palette + soft bloom glow, gentle
motion, long segments. Reads /tmp/dd2_field.bin (gs_deepdive2_gen).
Preview: python3 gs_deepdive2_film.py test <f>    Full: python3 gs_deepdive2_film.py"""
...
def roman(n):
    ...
    for v,sym in ROMAN:
        while n>=v: s+=sym; n-=v
    return s
M=json.load(open('/tmp/dd2_meta.json')); R=M['R']; MF=M['M']; PAT=M['patterns']; NP=len(PAT)
SZ=R*R*2
def readfield(idx,lf):
    g=idx*MF+min(MF-1,max(0,lf)); f=open('/tmp/dd2_field.bin','rb'); f.seek(g*SZ)
...
def window(idx,lf,t):
    ...
    col=relief(h01)
    bp=np.clip(col-165,0,255).astype(np.uint8)                          # soft bloom on the highlights
...
TITLE=110; SEG=MF; CODA=120; FADE=18
...
def compose(f):
    ...
    if kind=='title':
        ...
        label(cv,(W//2,560),"gray–scott · 512² · a slow descent",22,AC,0.7*g)
        text(cv,(W//2,840),"DEEP DIVE II",F_disp(126),BONE,g)
        text(cv,(W//2,990),"deeper into the most incredible form",F_ital(42),AC,g)
        label(cv,(W//2,1330),"eight worlds · two numbers each",19,DIM,0.7*g)
        return cv
    if kind=='coda':
        g=min(1,lf/20)*min(1,(CODA-1-lf)/16)
        label(cv,(W//2,560),"feed and kill · nothing else",20,AC,0.65*g)
        text(cv,(W//2,820),"two numbers,",F_disp(60),BONE,g)
        text(cv,(W//2,910),"endless form",F_disp(60),AC,g)
        label(cv,(W//2,1300),"cellautomata · deep dive ii",17,DIM,0.55*g)
        return cv
    ...
    label(cv,(W//2,1900),"cellautomata · deep dive ii",14,DIM,0.4)
    return cv
if __name__=='__main__' and len(sys.argv)>2 and sys.argv[1]=='test':
    compose(int(sys.argv[2])).convert("RGB").save('/tmp/dd2_test.png'); print("NF",NF,"patterns",NP); sys.exit()
# ===== FULL RENDER =====
silent="/tmp/dd2_silent.mp4"
...
total=NF/FPS; out="/tmp/web8_deepdive2.mp4"
