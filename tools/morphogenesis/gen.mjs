...
const STEM = process.env.GEN_OUT || ('/tmp/g_' + ID);   // override output stem (so multiple regimes of one rule can coexist)
...
fs.writeFileSync(`${STEM}_spr.json`,JSON.stringify(spr));
fs.writeFileSync(`${STEM}_meta.json`,JSON.stringify({id:ID,label:g.label,W,H,SC,frames,modes,pop}));
