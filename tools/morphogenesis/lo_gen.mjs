...
const WP=[
 ['mitosis',0.0367,0.0649],['spots',0.030,0.062],['turbulent',0.026,0.054],['worms',0.030,0.056],
 ['labyrinth',0.039,0.058],['stripes',0.042,0.060],['coral',0.0545,0.062],['u-skate',0.060,0.064],
 ['holes',0.046,0.0635],['negatons',0.039,0.0645],['spots',0.034,0.063],['mitosis',0.0367,0.0649]];
const NF=2880, SEG=WP.length-1;
setF(WP[0][1],WP[0][2]); g.reset();
const rng=mul(424242); for(let i=0;i<150;i++) g.paint((rng()*W)|0,(rng()*H)|0,5,'paint');
const fd=fs.openSync('/tmp/lo_field.bin','w'); const fk=[],names=[],pop=[]; const t0=Date.now();
for(let f=0;f<NF;f++){
  ...
  if(f%288===0) console.log('frame',f,((Date.now()-t0)/1000).toFixed(0)+'s',names[f],pop[f]);
}
fs.writeFileSync('/tmp/lo_meta.json',JSON.stringify({W,H,NF,SEG,WP,fk,names,pop}));
