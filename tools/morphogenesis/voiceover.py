...
def tts(text, voice, out):
    ...
    r = requests.post(f"{API}/{voice}", headers={"xi-api-key": KEY, "accept": "audio/mpeg",
        "content-type": "application/json"}, json={"text": text, "model_id": MODEL,
...
def main():
    ...
    if "--per-line" in sys.argv:
        for i, ln in enumerate(lines):
            tts(ln, voice, out.replace(".mp3", f"_{i:02d}.mp3"))
    else:
        tts(script.get("join", " … ").join(lines), voice, out)   # combined narration
