// OPALESCENCE — structural colour. A living membrane whose curved surfaces shimmer with
// thin-film interference (opal / nacre / beetle-shell). Path favours domes & curves so the
// iridescence has surfaces to play across: spots → mitosis → coral → labyrinth → u-skate → coral.
...
// Curved, domed, branching morphologies — all in the alive band.
const WP=[
 ['spots',0.030,0.062],
...
const rng=mul(90909090);
for(let i=0;i<100;i++) g.paint((rng()*W)|0,(rng()*H)|0,6,'paint');
const fd=fs.openSync('/tmp/opalescence_field.bin','w'); const fk=[],names=[],pop=[]; const t0=Date.now();
for(let f=0;f<NF;f++){
...
fs.closeSync(fd);
fs.writeFileSync('/tmp/opalescence_meta.json',JSON.stringify({W,H,NF,SEG,WP,fk,names,pop}));
