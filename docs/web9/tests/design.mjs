...
// web9 reuses web8's self-hosted font assets (no duplicated binaries) via a
// relative same-origin path — still self-hosted, never a runtime CDN.
for (const f of FONTS) assert(exists(`../web8/assets/fonts/${f}`),
  `missing self-hosted font ../web8/assets/fonts/${f} (web9 shares web8's fonts)`);
assert(/url\("\.\.\/web8\/assets\/fonts\//.test(css),
  "styles.css must @font-face the shared web8 fonts via ../web8/assets/fonts/");
assert(!/googleapis|jsdelivr|fonts\.gstatic/i.test(css), "styles.css must not pull fonts from a CDN");
