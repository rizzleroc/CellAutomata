...
// ── 8. web9 Instrument layer (observables + CSV + shareable run link) ────────
assert(exists("observables.js"), "web9 instrument module observables.js is missing");
if (exists("observables.js")) {
  try {
    execFileSync(process.execPath, ["--check", path.join(ROOT, "observables.js")], { stdio: "pipe" });
    ok();
  } catch (e) {
    fail(`syntax error in observables.js: ${String(e.stderr || e).slice(0, 160)}`);
  }
  assert(/export\s+const\s+Observables\s*=/.test(read("observables.js")),
    "observables.js does not export Observables");
}
assert(/from\s+["']\.\/observables\.js["']/.test(main), "main.js does not import ./observables.js");
assert(/Observables\.record\s*\(/.test(main), "main.js does not feed the instrument (Observables.record)");
assert(/Observables\.reset\s*\(/.test(main), "main.js does not reset the instrument per specimen");
for (const id of ["obsSpark", "obsRead", "obsCsvBtn", "obsLinkBtn"]) {
  assert(html.includes(`id="${id}"`), `index.html missing instrument element #${id}`);
}
// the amoeba "slime" guide layer must survive the fork
