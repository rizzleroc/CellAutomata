// cellauto · web9 controller — "The Instrument": web8's guided lab (the amoeba
// "slime" guide) + a live measurement layer (observables.js). Forked from web7's
// "Catalytic Silence" shell over web6's engine.
//
...
function selectExperiment(m) {
  ...
  Observables.reset(plateOf(m).name);                  // web9 instrument: a fresh time-series per specimen
...
function expTick(now) {
  ...
  if (advanced) {                                      // web9 instrument: measure each real step
...
// ── web9 Instrument layer: live observable readout + CSV export + run link ────
...
if (lab) {
  const boot = applyRunHash();      // web9: restore stage/view/palette from a shared run link
