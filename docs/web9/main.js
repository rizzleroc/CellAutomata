...
// ── Stage registry — canonical scientific order, locked to the engine's
// EXTENDED_STAGE_CLASSES: soup → vent → reaction-diffusion → mineral → RAF →
// chirality → RNA → code → coacervate → vesicles → selection → LUCA → life.
// web9 reorders web8's legacy "5 canonical stages first, extras appended"
// layout so the catalogue walks all 13 stages in the order the abiogenesis
// pipeline actually runs them. Order is pinned by tests/smoke.mjs.
const STAGES = [
...
function loadStage(m) {
  ...
  // web9: let the amoeba guide narrate this plate (guide.js listens on window).
  window.dispatchEvent(new CustomEvent('web9:stage', {
