...
| ④ | **Pro · 4000² export pill** — token-gated **in-page** hi-res SEM export (`pro.js` / `CatSilPro`); no server needed on Pages | over the SEM plate | `[DONE]` |
| ⑤ | **13-node timeline scrubber** — ◀/▶ + a node per stage, `◉` = current, click → `loadStage` | footer | `[DONE]` |
| ⑥ | **Provenance strip** — `PL.N · idx/13 · BUILD MK X` | footer, beside ⑤ | `[DONE]` |
| ⑦ | **Amoeba guide** (port web8 `guide.js`/`blobgeom.js`) | lower-left | `[OPT]` |
| ⑧ | **Server Pro export** — pill POSTs the current stage/params to a web9 `/api/render` (Clerk/Stripe, PR #76) | plate pill | `[OPT]` (superseded by ④'s in-page token export for the static deploy) |
...
| Change | Status | Notes |
|---|---|---|
| ① MK X build tag · ② magenta accent · ③ hero-art rail | `[DONE]` | shipped in PR #78 |
| ⑤ timeline scrubber · ⑥ provenance strip | `[DONE]` | in the Mark X client |
| ④ Pro 4000² pill — token-gated in-page SEM export (`pro.js`/`CatSilPro`, shared with slime) | `[DONE]` | deploy PR; static-Pages-safe |
| ⑦ amoeba guide · ⑧ server Pro `/api/render` · #65 control parity | `[OPT]` | stretch |
