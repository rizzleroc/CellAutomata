...
// 11. Mark X (web10) identity — build tag, Pro export pill, hero-art rail. -----
assert(/class="brand-build"/.test(html) && /MK/.test(html), "missing the Mark X build tag");
assert(/class="pro-pill"/.test(html) && /id="proPill"/.test(html) && /\.pro-pill/.test(css),
  "missing the Pro · 4000² export pill");
// Pro export is token-gated by the client-side CatSilPro module (docs/web10/pro.js),
// wired to an in-page hi-res SEM export (up to 4000²) — not a link to the web9 server.
assert(exists("pro.js") && /window\.CatSilPro/.test(read("pro.js")), "missing the CatSilPro token module (pro.js)");
assert(/<script src="\.\/pro\.js">/.test(html), "index.html must load pro.js before main.js");
assert(/CatSilPro/.test(main) && /exportProPlate/.test(main) && /4000/.test(main),
  "main.js must wire the Pro pill to the token-gated 4000² SEM export");
