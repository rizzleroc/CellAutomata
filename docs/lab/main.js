...
// ── Pro · hi-res SEM plate export (token-gated by CatSilPro) ──────────────────
// The pill renders the CURRENT stage's micrograph at up to 4000×4000 and offers
// it as a PNG. Grid rules go through the SEM depth-shade at its max supersample,
// then upscale to the plate; the photoreal LIFE feed renders straight to size.
// 4000 is a hard cap (cf. SEC-008 / #42 / standing requirement #1).
const PRO_PLATE_SIZE = 4000;
function slugify(s) { return (s || 'plate').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'plate'; }

function exportProPlate() {
  if (!expRule) { announce('No live micrograph to export on this plate.'); return; }
  const SIZE = PRO_PLATE_SIZE;
  const off = document.createElement('canvas');
  off.width = SIZE; off.height = SIZE;
  const octx = off.getContext('2d');
  if (!octx) { announce('Hi-res export is unavailable in this browser.'); return; }

  if (expRule.hiRes && typeof expRule.renderPhotoreal === 'function') {
    // Photoreal LIFE feed: render straight into the plate-sized canvas.
    expRule.renderPhotoreal(octx, SIZE, SIZE,
      (w, h) => { const c = document.createElement('canvas'); c.width = w; c.height = h; return c; });
  } else if (window.SEM && typeof expRule.renderHeight === 'function') {
    const w = expRule.width, h = expRule.height;
    const S = Math.max(1, Math.min(4, Math.floor(SIZE / Math.max(w, h)) || 1));   // supersample, capped 1–4
    const sw = w * S, sh = h * S;
    const buf = new Uint8ClampedArray(sw * sh * 4);
    expRule.renderHeight(expHeightBuf);
    SEM.render(expHeightBuf, w, h, buf, { palette: expPalette, scale: S });
    const src = document.createElement('canvas'); src.width = sw; src.height = sh;
    src.getContext('2d').putImageData(new ImageData(buf, sw, sh), 0, 0);
    octx.imageSmoothingEnabled = true; octx.imageSmoothingQuality = 'high';
    octx.drawImage(src, 0, 0, SIZE, SIZE);
  } else {
    announce('This plate cannot be exported.'); return;
  }

  const name = slugify(currentMeta ? plateOf(currentMeta).name : 'plate');
  off.toBlob((b) => {
    if (!b) { announce('Hi-res export failed — the plate could not be encoded.'); return; }
    const url = URL.createObjectURL(b), a = document.createElement('a');
    a.href = url; a.download = `catalytic-silence-${name}-sem-${SIZE}.png`;
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(() => URL.revokeObjectURL(url), 4000);
    announce(`Exported the ${plateOf(currentMeta).name} plate at ${SIZE}×${SIZE}.`);
  }, 'image/png');
}

const proPill = $('proPill');
if (proPill && window.CatSilPro) {
  const reflectPro = (unlocked) => {
    proPill.classList.toggle('owned', !!unlocked);
    proPill.textContent = unlocked ? 'PRO · 4000² EXPORT ↓' : 'PRO · 4000² EXPORT ↓';
    proPill.title = unlocked
      ? 'Export this stage as a 4000×4000 SEM micrograph plate'
      : 'Unlock Pro to export this stage as a hi-res SEM micrograph plate (up to 4000×4000)';
  };
  reflectPro(CatSilPro.isUnlocked());
  CatSilPro.onChange(reflectPro);
  proPill.addEventListener('click', () => {
    if (CatSilPro.isUnlocked()) { exportProPlate(); return; }
    CatSilPro.showPaywall({ onUnlock: () => exportProPlate() });
  });
} else if (proPill) {
  // Pro module absent (shouldn't happen) — degrade to a direct export.
  proPill.addEventListener('click', exportProPlate);
}
