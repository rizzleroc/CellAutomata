...
(function () {
  ...
  function make() {
    ...
    return {
      ...
      // v4.1 sprite layer — self-replicating spots as protocell spheres.
      // Local maxima of v ≥ SPOT_THRESHOLD become bone-coloured 3-D
      // domes catching light.  Scan on a stride so we don't emit one
      // sprite per pixel.
      sprites() {
        const out = [];
        const SPOT_THRESHOLD = 0.30;
        const stride = 4;
        for (let y = stride; y < H - stride; y += stride) {
          for (let x = stride; x < W - stride; x += stride) {
            const ic = y * W + x;
            const me = v[ic];
            if (me < SPOT_THRESHOLD) continue;
            // Reject unless it's a maximum in its 3x3 stride-window.
            let isMax = true;
            for (let dy = -stride; dy <= stride && isMax; dy += stride) {
              for (let dx = -stride; dx <= stride && isMax; dx += stride) {
                if (dx === 0 && dy === 0) continue;
                if (v[(y + dy) * W + (x + dx)] > me) isMax = false;
              }
            }
            if (!isMax) continue;
            // Scale follows the spot's "v" value — saturated spots are
            // bigger, marginal ones smaller.
            const scale = 1.6 + (me - SPOT_THRESHOLD) * 6;
            out.push({ kind: "protocell-sphere", x: x + 0.5, y: y + 0.5, scale });
          }
        }
        return out;
      },
