...
(function () {
  ...
  function drawLifePhotoreal(ctx, CW, CH, state, atlas, frame, newCanvas) {
    ...
    for (let k = 0; k < list.length; k++) {
      ...
      // ``frame`` is wall-clock SECONDS. Flipbook breathes at ~4 frames/s
      // (≈2.5 s per 10-frame loop); body sway is a slow ~0.6 rad/s drift.
      const fidx = (((Math.floor(frame * 4) + o.oid * 3) % F) + F) % F;
      const isdiv = o.oid === divOid && frac > 0.5;
      const img = isdiv ? atlas.div : atlas.sheet;
      const sy = isdiv ? 0 : (o.oid % V) * tile;
      const sx = fidx * tile;
      const ang = ((o.facing * 45) + 4 * Math.sin(frame * 0.6 + o.oid)) * Math.PI / 180;
      ctx.save();
      ctx.translate(cx, cy);
      ctx.rotate(ang);
      ...
      for (let c = 0; c < ncil; c++) {
        ...
        // Metachronal beat wave at ~1.2 Hz (7.5 rad/s) — a visible undulation
        // travelling around the rim, calm but clearly alive.
        const ph = frame * 7.5 - (c / ncil) * 6.2832 * 3;
  ...
  function make() {
    ...
    return {
      ...
      width: W,
      height: H,
      // Photoreal sprite render (the capstone LIFE feed). The web5 driver
      // routes here when the atlas is ready, on a hi-res square canvas.
      hiRes: true,
      // Slow real-time sim cadence (the driver reads this): the engine steps
      // ~5×/s so cells crawl and divide believably instead of racing.
      stepsPerSec: 5,
      atlasReady: function () { return LIFE_ATLAS.ready; },
      renderPhotoreal: function (ctx, CW, CH, newCanvas) {
        // Drive cilia/flipbook off a real-time SECONDS clock (not a per-frame
        // counter), so animation speed is independent of frame rate AND of the
        // slow sim cadence — smooth and calm at any refresh rate.
...
// ── web5 driver — the live, real-time Stage XIII photoreal LIFE feed ────────
// Self-contained: instantiates the engine above, steps it at its calm
// stepsPerSec cadence, and blits the photoreal plate every animation frame so
// cilia/flipbook stay butter-smooth (their motion is clocked off real time,
// not the slow sim). Mirrors the verified web4 routing — minus the lab shell.
(function () {
  "use strict";
  const canvas = document.getElementById("life-canvas");
  if (!canvas || !window.CA || !CA.RULES || !CA.RULES.life) return;
  const ctx = canvas.getContext("2d");
  const readout = document.getElementById("life-readout");
  const playBtn = document.getElementById("life-play");
  const newCanvas = function (w, h) {
    const c = document.createElement("canvas"); c.width = w; c.height = h; return c;
  };

  const rule = CA.RULES.life();
  rule.reset();
  const SPS = rule.stepsPerSec || 5;     // calm real-time sim cadence

  let running = false, raf = 0, last = 0, statT = 0;
  function frame(now) {
    if (!running) { raf = 0; return; }
    if (!last) last = now;
    const interval = 1000 / SPS;
    let safety = 4;
    while (now - last >= interval && safety-- > 0) { rule.step(); last += interval; }
    rule.renderPhotoreal(ctx, canvas.width, canvas.height, newCanvas);
    if (readout && now - statT > 600) { readout.textContent = rule.population(); statT = now; }
  ...
  if (playBtn) playBtn.addEventListener("click", function () { running ? pause() : play(); });

  // Paint one frame immediately (placeholder until the atlas PNGs load), then
  // start the loop. The renderer handles the atlas-still-loading case itself.
  rule.renderPhotoreal(ctx, canvas.width, canvas.height, newCanvas);
  play();
})();
