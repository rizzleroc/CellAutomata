...
| `web9/` | **The Instrument** = web8 (guide/"slime" layer included) + a live measurement layer (`observables.js`): per-step observable sparkline (roughness σ²/⟨h⟩), CSV export, and a shareable run-URL (stage/view/palette in the hash). A **registration-gated paywall** (`paywall.js`/`paywall.css`) locks the Parameters rail (tweak knobs · step/reset transport · CSV/link export) until you **register (name + email) and claim an access code** — the **first 10 registrants get a code free** (ten pre-minted, checksum-verifiable `CS-REG-XXXX-CCCC` codes tracked in a `localStorage` ledger); once claimed the free tier closes and a code is `$9.99` (demo checkout). Any valid code (free, minted, or a legacy token) redeems on any device; watching the live specimen is free. The landing links out to the full-feed **Studio** (the Pro 4K artifact), the on-site **Slime Studio**, and the **Murmuration** simulator | active |
